...
/*int getFeatureMaps_neon(unsigned char* src_data, int width, int height, st_fhog_feture **map)
{
    ...
    float *dx = (float *)malloc(sizeof(float)*(image->width * image->width));
    memset(dx, 0, sizeof(float)*(image->width * image->width));
    float *dy = (float *)malloc(sizeof(float)* (image->width * image->width));
    memset(dy, 0, sizeof(float)*(image->width * image->width));
